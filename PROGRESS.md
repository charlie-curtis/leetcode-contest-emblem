...
- 2026-07-11: Removed the `virtual` contest mode end-to-end (it always 501'd since LeetCode doesn't expose virtual history via the rated-history GraphQL query) — dropped the mode switcher, `/actual.svg` and `/virtual.svg` shortcut routes, `?mode=` query handling, `CONTEST_MODE` env var, and the `contestMode.js` module. Also enlarged the rating trend chart and fixed the header/axis-label overlap it had.
